...
class LinkPurpose {
  ...
  constructor (option) {
    ...
    LinkPurpose.findLinks = function () {
      ...
      const selector = `:is(${LinkPurpose.options.base.selector}${shadowSelector})${LinkPurpose.options.ignore}`
      ...
      if (LinkPurpose.options.shadowComponents) {
        const subSelector = `:is(${LinkPurpose.options.base.selector})${LinkPurpose.options.ignore}`
    ...
    LinkPurpose.processLinks = function () {
      ...
      if (LinkPurpose.options.domain) {
        internalUrls = internalUrls ? internalUrls + `, ${LinkPurpose.options.domain}` : LinkPurpose.options.domain
      }
      ...
      LinkPurpose.marks.forEach((mark) => {
        mark.hits.forEach((hit, i) => {
          ...
          if (i === 0) {
            ...
            iconSpan.classList.add(LinkPurpose.options.base.iconWrapperClass, LinkPurpose.options[hit].iconWrapperClass)
            // TODO 'classes'
            if (LinkPurpose.options[hit].iconType === 'html') {
              iconSpan.innerHTML = LinkPurpose.options[hit].iconHTML
            } else if (LinkPurpose.options[hit].iconType === 'classes') {
              console.log('classes!')
              LinkPurpose.options[hit].iconClasses.forEach((cls) => {
                iconSpan.classList.add(cls)
              })
            } else if (LinkPurpose.options[hit].iconType === 'src') {
              const image = document.createElement('img')
              image.setAttribute('src', LinkPurpose.options[hit].iconSRC)
            }
            ...
            iconText.classList.add(LinkPurpose.options[hit].iconWrapperClass)
            iconText.textContent = iconText.textContent + LinkPurpose.options[hit].message
          }
        })
      })
    }
